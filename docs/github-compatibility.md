...
- The README uses GitHub's supported `<picture>` plus `prefers-color-scheme` behavior for automatic light/dark asset selection. Reduced-motion sources appear first so they win when both motion and color preferences match. The appearance control, systems, architecture, and signal scenes use separate mobile sources at the GitHub-column-aware 1080 px breakpoint.
- The visible README body remains image-only: five `<picture>` blocks with no native Markdown headings, paragraphs, lists, badges, separators, or text between them. The first picture is the sole link and opens GitHub's official Appearance settings; it is not an unsupported in-place theme toggle. Descriptive `alt` text remains for accessibility.
