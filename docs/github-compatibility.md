...
- The README uses four dark-only `<picture>` blocks. There is no theme control, linked button, light asset, or `prefers-color-scheme` source; every SVG carries its own opaque dark canvas so the composition stays dark in every GitHub appearance mode.
...
- The visible README body remains image-only, with no native Markdown headings, paragraphs, lists, badges, separators, links, or text between the four pictures. Descriptive `alt` text remains for accessibility.
