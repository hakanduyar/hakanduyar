...
data/telemetry.json          <- committed snapshot; single source of every number
        ...
        v
in-memory scene graph        <- 20 RenderedAssets, each with a text manifest
        |                       |
...
| `src/build.ts` | The scene graph: eight panels x two themes, with animated/static variants for identity and signal, and `PANEL_IDS`, the contract the validators and the README assembler both check themselves against |
...
V3 restores constrained motion on two panels. Identity uses the named
`identity-acquire` and `identity-pulse` effects, and signal uses
`signal-scan`; their animated variants use unguarded CSS keyframes, while
their static variants contain no motion CSS. The README puts the static
variants first in each panel's reduced-motion `<picture>` ladder. Focus, the
four system plates and channels remain static two-source dark/light pairs.

The engine is constrained by construction: `Canvas` exposes only the named
motion register, animated mode is used only by identity and signal, and the
validators reject transitions, SMIL, reduced-motion queries and effects
outside each panel's allowlist. The resulting build is 20 SVG files: four
variants each for identity and signal, plus two theme files for each of the
other six panels.
...
| `npm run render` | emit all 20 assets (`-- --check` = drift gate only) |
...
| `npm run qa:visual` | headless-Chrome screenshots at 890px and 360px + static stillness and animation liveness proof |
