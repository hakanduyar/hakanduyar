...
function profileImage(name: string, alt: string, animated = false, responsive = false): string {
  ...
  const mobileStaticAttrs = animated && responsive
    ...
    : '';
  return `<img class="profile-visual" data-desktop="${base}-dark.svg"${staticAttrs}${mobileAttrs}${mobileStaticAttrs} src="${base}-dark.svg" alt="${escapeHtml(alt)}" align="top">`;
}
...
${profileImage('hero', 'Hakan Duyar circular identity field with Flight, Signal, and Spatial modes.', true)}<details class="profile-details"><summary><picture><source media="(max-width: 1080px)" srcset="../assets/generated/expand-mobile-dark.svg"><img class="expand-visual" src="../assets/generated/expand-dark.svg" alt="Show systems, architecture, and public signal" width="95%" align="middle"></picture></summary>${profileImage('systems', 'Four selected systems arranged on a connected engineering path.', true, true)}${profileImage('architecture', 'Interface, state, services, and delivery shown as spatial architecture layers.', true, true)}${profileImage('signal', 'Measured 52-week public contribution signal and language distribution.', true, true)}</details>
</div></article>
...
let theme=params.get('theme')==='dark'?'dark':'light';let mobile=params.get('mobile')==='1';let reduced=params.get('motion')==='reduce';let clean=params.get('clean')==='1';const expanded=params.get('expanded')==='1';document.querySelector('.profile-details').open=expanded;
